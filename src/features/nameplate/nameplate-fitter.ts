import { ELLIPSIS, NAMEPLATE_SETTINGS, NAMEPLATE_TOKEN_FLAGS } from "./constants";
import { MODULE_ID as TOOLKIT_MODULE_ID } from "../../constants";
...
export class NameplateFitter {
    ...
    static apply(token: Token, force = false): void {
        try {
            ...
            console.warn("imaginaryfriend-toolkit | Failed to fit nameplate for token", token, error);
        }
    }
    ...
    private static _apply(token: Token, force: boolean): void {
        ...
        const enabled = game.settings.get(TOOLKIT_MODULE_ID, NAMEPLATE_SETTINGS.ENABLED.key) as boolean;
        const colorByDisposition = game.settings.get(
            TOOLKIT_MODULE_ID,
            NAMEPLATE_SETTINGS.COLOR_BY_DISPOSITION.key
        ) as boolean;
        const disabledForToken = document.getFlag(TOOLKIT_MODULE_ID, NAMEPLATE_TOKEN_FLAGS.DISABLE_AUTOFIT) === true;
        ...
        const minFontSize = game.settings.get(TOOLKIT_MODULE_ID, NAMEPLATE_SETTINGS.MIN_FONT_SIZE.key) as number;
        const shrinkStep = game.settings.get(TOOLKIT_MODULE_ID, NAMEPLATE_SETTINGS.FONT_SHRINK_STEP.key) as number;
        const maxLines = game.settings.get(TOOLKIT_MODULE_ID, NAMEPLATE_SETTINGS.MAX_LINES.key) as number;
